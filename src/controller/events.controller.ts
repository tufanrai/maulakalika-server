...
import fs from "fs";
import NewsAndEvents from "../model/events.model";
...
export const getAllFiles = asyncHandler(async (req: Request, res: Response) => {
  const files = await NewsAndEvents.find();
...
export const getSpecificFile = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    const file = await NewsAndEvents.findById(id);
...
export const uploadFile = asyncHandler(async (req: Request, res: Response) => {
  ...
  console.log(result);
  // Save to MongoDB
  const uploadData = await NewsAndEvents.create({
...
export const updateFile = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = await NewsAndEvents.findById(id);
